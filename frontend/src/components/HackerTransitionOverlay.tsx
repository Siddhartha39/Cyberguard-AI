...
import { Cpu, Lock, Terminal, Activity, Eye, Shield, Sparkles, Zap } from 'lucide-react';
...
export const HackerTransitionOverlay: React.FC<HackerTransitionOverlayProps> = ({ onComplete }) => {
  ...
  const [hexCode, setHexCode] = useState('0x0290AC3C');
  const canvasRef = useRef<HTMLCanvasElement | null>(null);

  const logs = [
    { text: '[>] BOOTING CYBERGUARD CORE V2.0 KERNEL...', icon: <Cpu size={15} color="#00f0ff" /> },
    { text: '[+] INITIALIZING SECURE NEURAL SANDBOX ENCLAVE...', icon: <Lock size={15} color="#38bdf8" /> },
    { text: '[+] ESTABLISHING REAL-TIME RDAP / WHOIS PROTOCOL...', icon: <Terminal size={15} color="#00ff88" /> },
